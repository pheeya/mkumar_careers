...
req.onload = function (e) {
  ...
  for (let i = 0; i < json_sheet.length; i++) {
    ...
    // academic level
    var academicImage = document.createElement("img");
    academicImage.classList.add("academicImage")
    card.appendChild(academicImage);
    // image div
    ...
    // card.href = s.Link;
    // card.target = "_blank"
    card.dataset.thumbnail = s["Discipline Image"];
    card.dataset.discipline = s["Discipline"];
...
function create_filters(sheet) {
  ...
  options.forEach(function (fetched_option) { //option = filter label
    ...
    if (fetched_option === "Posted By" || fetched_option === "Discipline" || fetched_option === "Country" || fetched_option === "Academic Level") {
